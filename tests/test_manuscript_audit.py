...
class TestPropofolConditionNumber:
    @pytest.fixture(autouse=True)
    def load_data(self):
        self.d = _load("amplification_propofol.json")
        self.g = self.d["group_statistics"]

    def test_condition_number_shift(self):
        s = self.g["delta_condition_number"]
        assert abs(s["cohens_d"] - (-0.723)) < 0.01
        assert s["p_ttest"] < 0.005
        assert s["n_negative"] >= 17


class TestPropofolStabilityFraction:
    @pytest.fixture(autouse=True)
    def load_data(self):
        self.d = _load("amplification_propofol.json")
        self.s = self.d["group_statistics"]["stability_summary"]

    def test_awake_fraction_stable(self):
        assert abs(self.s["awake_fraction_stable_mean"] - 0.894) < 0.01

    def test_sed_fraction_stable(self):
        assert abs(self.s["sed_fraction_stable_mean"] - 0.670) < 0.01


class TestIEEGAmplificationDissociation:
    @pytest.fixture(autouse=True)
    def load_data(self):
        self.d = _load("transient_amplification.json")
        self.c = self.d["cross_subject_correlations"]

    def test_condition_number_vs_kreiss(self):
        s = self.c["condition_number_vs_kreiss"]
        assert abs(s["r"] - 0.912) < 0.01
        assert s["p"] < 1e-6

    def test_ep_score_vs_kreiss_null(self):
        s = self.c["ep_score_vs_kreiss"]
        assert abs(s["r"]) < 0.1
        assert s["p"] > 0.5

    def test_branching_ratio_vs_kreiss_null(self):
        s = self.c["branching_ratio_vs_kreiss"]
        assert abs(s["r"]) < 0.25
        assert s["p"] > 0.3


class TestPACCorrelations:
    @pytest.fixture(autouse=True)
    def load_data(self):
        self.d = _load("cross_frequency.json")
        self.c = self.d["correlations"]

    def test_mi_vs_tau(self):
        s = self.c["mi_vs_tau"]
        assert abs(s["r"] - 0.526) < 0.01
        assert abs(s["p"] - 0.025) < 0.005

    def test_mi_vs_sigma_null(self):
        s = self.c["mi_vs_sigma"]
        assert abs(s["r"]) < 0.1
        assert s["p"] > 0.5

    def test_mi_vs_lzc_null(self):
        s = self.c["mi_vs_lzc"]
        assert abs(s["r"]) < 0.15
        assert s["p"] > 0.5
