...
Runs the identical multi-run pooled pipeline on every subject sequentially.
Each subject's 5 runs (DurR1-DurR5) are pooled before analysis.
Results are saved per-subject and aggregated into a group summary.
"""
...
def main():
    ...
    log(f"{'='*70}")
    log(f"CMCC BATCH PIPELINE - {len(all_subjects)} subjects")
    log(f"{'='*70}")
